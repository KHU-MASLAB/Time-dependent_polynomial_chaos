function out = SolveMonteCarlo_par(opts)
...
parpool(opts.nWorkers)
rv = parallel.pool.Constant(rv_M);
xi = parallel.pool.Constant(xi_M);
c = parallel.pool.Constant(@() fopen(tempname(pwd),'wt'),@fclose);
spmd
    A = (fopen(c.Value)); 
end
parfor i = 1:opts.nSample
    [Prop,init] = SinglePendulum("h",h,"endTime",endTime,rvName,rv.Value(i),"th1_0",th1_0);
    [~, X] = ode45(@(t,x) SolveDAE_SinglePendulum(t,x,Prop), Prop.tspan, init, ode_opt);
    
    xi_i = [xi.Value(i), zeros(1,size(X,1)-1)];
    fprintf(c.Value,[repmat('%.6f,\t',1,size(X,1)-1) '%.6f\t' '\n'],xi_i);
    fprintf(c.Value,[repmat('%.6f,\t',1,size(X,1)-1) '%.6f\t' '\n'],X); % Write results
